...
router.get('/active', async (req,res,next) => {
  try {
    const { rows } = await pool.query(`
      ...
      ORDER BY b.end_time, se.start_time
    `)
    res.json(rows)
  } catch (err) { next(err) }
})
...
router.post('/start', async (req,res,next) => {
  ...
  try {
    ...
    const now = new Date()
    const startsAt = new Date(booking.start_time)
    const endsAt = new Date(booking.end_time)
    const earliestStart = new Date(startsAt.getTime() - EARLY_START_MINUTES * 60 * 1000)

    if (now < earliestStart) {
      throw Object.assign(new Error(`This session can only be started up to ${EARLY_START_MINUTES} minutes before the booking time.`),{status:409})
    }
    if (endsAt <= now) throw Object.assign(new Error('This booking time has already ended.'),{status:409})
    ...
    if (!station.rows[0]) throw Object.assign(new Error('Station not found.'),{status:404})
    ...
    req.app.get('io').emit('session:started',{stationId:booking.station_id,bookingId:booking.id,sessionId:result.rows[0].id})
...
router.post('/:id/end', async (req,res,next) => {
  ...
  try {
    ...
    req.app.get('io').emit('session:ended',{stationId:session.station_id,bookingId:session.booking_id,sessionId:session.id})
